...
def Get_interface(router_name: str, liens: dict, masque) -> str:
    res = ""
    for interface in INTERFACE.keys():
        if interface not in liens.keys():
            res += NOT_set_INTERFACE.get(interface, "")
        else:
            res += get_interface_txt(router_name, INTERFACE[interface], liens[interface][0], masque)
    return res



def Get_end(isBorder: bool, name: str) -> str:
    id = name[-1]
    res = f"""\
ipv6 router ospf 4
